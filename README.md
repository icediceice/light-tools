...
Lines that differ only by a varying token — a counter, a pid, a uuid, a duration — collapse into one template row carrying the values that actually changed, so a climbing restart counter stays visible instead of hiding inside a wall of near-identical lines.

Every distinct line kind is rendered, **including one that occurs exactly once**. A verdict — `BUILD FAILED`, `panic:`, `exit status 1` — is a singleton by nature, and it is the line you came for.

`light_bash`, `light_ssh` and `light_ops` all do this.
